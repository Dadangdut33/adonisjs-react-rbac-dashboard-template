...
import { Link as LinkIcon, Music2, Pencil, Save, X } from 'lucide-react'
...
const attrsToForm = (attrs: AudioAttachmentAttrs): FormState => ({
  url: attrs.url || '',
  name: attrs.name || '',
...
export default function AudioAttachmentNodeView({ node, editor, updateAttributes }: NodeViewProps) {
  const [isEditable, setIsEditable] = useState(!!editor?.isEditable)
  const [isEditing, setIsEditing] = useState(false)
  const attrs = node.attrs as AudioAttachmentAttrs
      ...
      <CardContent className="space-y-3 px-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-heading">{merged.name || 'Untitled Audio'}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              {merged.mimeType || 'audio file'}
            </p>
            <div className="mt-2 flex flex-wrap gap-2">
              <Badge variant="neutral">Audio</Badge>
        ...
        {merged.url ? <AudioPlayer src={merged.url} mimeType={merged.mimeType} /> : null}
      </CardContent>
    </Card>
  )
    ...
    <NodeViewWrapper className="not-prose my-3">
      ...
      {isEditing && isEditable ? (
        <div className={cn('mb-3 grid gap-2 rounded-base border-2 border-border bg-secondary-background p-3', widthClass, positionClass)}>
          <Input value={form.url} onChange={(e) => updateField('url', e.target.value)} placeholder="Audio URL" />
          <Input value={form.name} onChange={(e) => updateField('name', e.target.value)} placeholder="Audio name" />
          <Input value={form.mimeType} onChange={(e) => updateField('mimeType', e.target.value)} placeholder="MIME type (audio/mpeg)" />
          <Input value={form.extension} onChange={(e) => updateField('extension', e.target.value)} placeholder="Extension (mp3, wav...)" />
